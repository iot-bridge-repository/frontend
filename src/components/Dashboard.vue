...
const notifCount = ref(3); // Ganti dengan reactive data kamu, misalnya dari API
...
const ponds = ref([{ name: "KOLAM A - RAS" }, { name: "KOLAM B - RAS" }, { name: "KOLAM C - Non RAS" }, { name: "KOLAM D - Non RAS" }]);
</script>
...
<template>
    ...
    <div class="mt-7 mb-4">
      <ul class="ml-4">
        <li class="mb-2 px-4 py-4 mr-4 flex flex-row border-gray-100 hover:bg-gray-100 group rounded-lg">
          <span>
            <svg class="fill-current h-5 w-5 text-[#FF6500] group-hover:text-[#28527A]" xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16">
        ...
        <li class="mb-2 px-4 py-4 mr-4 flex flex-row border-gray-100 hover:bg-gray-100 group rounded-lg">
          <span>
            <svg class="fill-current h-5 w-5 text-[#FF6500] group-hover:text-[#28527A]" viewBox="0 0 16 16">
              <path
  ...
  <main class="bg-white">
    <!-- Navbar -->
    <div class="fixed top-0 right-0 flex items-center w-[1093px] bg-white px-4 py-2 rounded-t-xl shadow-md z-50">
      <div class="flex items-center space-x-2">
        <button class="text-[#28527A]">&#9776;</button>
        <h1 class="text-[#28527A] font-poppins font-bold !text-xl">Dashboard</h1>
      </div>
    ...
    <!-- Pond Cards -->
    <div class="mt-10 pl-60 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-2 gap-10">
      <div v-for="pond in ponds" :key="pond.name" class="w-110 h-64 bg-[#28527A] text-white rounded-lg p-6 flex flex-col items-center justify-center shadow-md hover:bg-[#FF6500] transition">
        <h2 class="mb-5 text-xl font-bold text-center font-poppins">{{ pond.name }}</h2>
        <img src="../assets/ikan-ikon.svg" class="w-30 h-30" alt="Ikon Ikan" />
      </div>
    </div>
  </main>
  <!-- CONTENT -->
